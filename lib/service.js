...
exports.setup = function(transport, config, db, testBucket) {
    ...
    service.on("queue", sendSchema, enqueue);
    service.on("send", sendSchema, send);
    service.on("blockEmail", s.Object({email : s.Email()}), blockEmail);
    service.on("blockToken", s.Object({token : s.String()}), blockToken);
    service.on("unblockToken", s.Object({token : s.String()}), unblockToken);
    service.on("unblockEmail", s.Object({email : s.Email()}), unblockEmail);
    ...
    function deliverEmail(task, callback) {
        transporter.sendMail(task, function(err, info) {
            ...
            if(testBucket) {
                ...
                callback(null, false); //delivered, remove from the queue
            }
        });
    }
